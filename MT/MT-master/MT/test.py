    ...
    if use_attn:
        for di in range(max_length):
            ...
            if ni == EOS_token:
                ...
                decoded_words.append(lang_dataset.output_lang.index2word[int(ni.data.cpu().numpy())])
...
if use_attn:
    decoder = AttentionDecoderRnn(hidden_size, output_size, n_layers=2)
...
# -------------------------------------
# 这是为了展示一下decoder attention的样子  所以又随便选取了一句话   上面选了10句话来展示翻译的效果
if use_attn:
    pair_idx = random.choice(list(range(len(lang_dataset))))
